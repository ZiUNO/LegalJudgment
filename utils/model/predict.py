...
def __get_label_ids(preds, threshold=-0.6):  # FIXME 调整阈值
    preds = preds.view(-1)
    preds_max = preds.max()
...
def predict_ids(model_version, sentence, labels_threshold=-0.6, highlight_threshold=0.01):
    do_lower_case = True
    ...
    preds_labels_ids = __get_label_ids(preds=preds, threshold=labels_threshold)
    preds_highlight_ids = __get_highlight_ids(attentions=attentions, threshold=highlight_threshold)

    return preds_labels_ids, preds_highlight_ids


def predict(sentence, label_cls="pred-text", highlight_cls="pred-text-highlight",
            model=r"./torch_pretrained_bert_multi_label/tmp/self/", labels_threshold=-0.6, highlight_threshold=0.01):
    label_ids, highlight_ids = predict_ids(model_version=model, sentence=sentence, labels_threshold=labels_threshold, highlight_threshold=highlight_threshold)
    label_list = __get_labels()
    labels = [label_list[ids] for ids in label_ids]
    sentence_format = "<text class='{highlight_cls}'>{word}</text>"
    highlight_sentence = "".join(
        [sentence_format.format(**{"highlight_cls": highlight_cls, "word": word})
         if i in highlight_ids else word for i, word in enumerate(sentence)])
    sentence = "%s%s%s" % (r"<text class='%s'>" % label_cls, highlight_sentence, r"</text>")
    sentence = re.sub(u"</text><text class='%s'>" % highlight_cls, u"", sentence)
...
if __name__ == '__main__':
    ...
    labels, highlight_sentence = predict(sentence=sentence,
