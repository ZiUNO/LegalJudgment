...
def __tsv2dataset(tsv):
    datasets = {"X": [], "y": []}
    charge_list = __get_charge_labels()
    charge_len = len(charge_list)
    article_list = __get_articles_labels()
...
if __name__ == '__main__':
    ...
    datasets = {key: __tsv2dataset(__read_tsv(datasets[key])) for key in datasets}
    ...
    y_pred_0 = clf.predict(X_test[0:1])
    # print("*" * 10 + " x train " + "*" * 10)
    # print(X_train)
    # print("*" * 10 + " y train " + "*" * 10)
    # print(y_train)
    ...
    print(y_test[0:1])
    print("-" * 20)
    print("score: %f" % clf.score(clf.predict(X_test), y_test))
    print("cost time: %d(s)" % (time() - start_time))
    joblib.dump(clf, "svm_clf.m")
    # print(X_test)
