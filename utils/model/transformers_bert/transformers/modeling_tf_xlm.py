...
class TFXLMPreTrainedModel(TFPreTrainedModel):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained model.
    """
...
XLM_START_DOCSTRING = r"""    The XLM model was proposed in
    ...
    Note on the model inputs:
        TF 2.0 model accepts two formats as inputs:

            - having all inputs as keyword arguments (like PyTorch model), or
...
XLM_INPUTS_DOCSTRING = r"""
    Inputs:
        ...
        **langs**: (`optional`) ```Numpy array`` or ``tf.Tensor`` of shape ``(batch_size, sequence_length)``:
            ...
            provided in the configuration of the model (only provided for multilingual model).
